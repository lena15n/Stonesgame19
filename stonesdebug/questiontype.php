...
/**
 * stonesdebug QUESTION TYPE CLASS
 *
 ...
 * @subpackage questiontypes
 */
class question_stonesdebug_qtype extends question_shortanswer_qtype {
    ...
    function get_question_options(&$question) {
        ...
        // Note: question_stonesdebug is an extension of the answer table rather than
        ...
        if (!$question->options->answers = get_records_sql(
                                ...
                                "     {$CFG->prefix}question_stonesdebug n " .
                                "WHERE a.question = $question->id " .
                                "    AND   a.id = n.answer " .
                                "ORDER BY a.id ASC")) {
            notify('Error: Missing question answer for stonesdebug question ' . $question->id . '!');
            return false;
        }
        $this->get_stonesdebug_units($question);
        ...
        if ($defaultunit = $this->get_default_stonesdebug_unit($question)) {
    ...
    function get_stonesdebug_units(&$question) {
        if ($units = get_records('question_stonesdebug_units',
    ...
    function get_default_stonesdebug_unit(&$question) {
    ...
    function save_question_options($question) {
        ...
        if (!$oldoptions = get_records('question_stonesdebug', 'question', $question->id, 'answer ASC')) {
            $oldoptions = array();
        }

        // Save the units.
        $result = $this->save_stonesdebug_units($question);
        ...
        foreach ($question->answer as $key => $dataanswer) {
            ...
            if (isset($options->id)) { // reusing existing record
                if (! update_record('question_stonesdebug', $options)) {
                    $result->error = "Could not update quiz stonesdebug options! (id=$options->id)";
                    return $result;
                }
            } else { // new options
                if (! insert_record('question_stonesdebug', $options)) {
                    $result->error = "Could not insert quiz stonesdebug options!";
                    return $result;
                }
            }
        }
        ...
        if (!empty($oldoptions)) {
            foreach($oldoptions as $oo) {
                delete_records('question_stonesdebug', 'id', $oo->id);
            }
        }
    ...
    function save_stonesdebug_units($question) {
        $result = new stdClass;

        // Delete the units previously saved for this question.
        delete_records('question_stonesdebug_units', 'question', $question->id);
        ...
        foreach ($question->multiplier as $i => $multiplier) {
            ...
            if (!empty($question->multiplier[$i]) && !empty($question->unit[$i])) {
                ...
                if (! insert_record('question_stonesdebug_units', $units[$i])) {
    ...
    function delete_question($questionid) {
        delete_records("question_stonesdebug", "question", $questionid);
        delete_records("question_stonesdebug_units", "question", $questionid);
        return true;
    }
    ...
    function get_correct_responses(&$question, &$state) {
        ...
        $unit = $this->get_default_stonesdebug_unit($question);
    ...
    function backup($bf,$preferences,$question,$level=6) {
        ...
        $stonesdebugs = get_records('question_stonesdebug', 'question', $question, 'id ASC');
        //If there are stonesdebugs
        if ($stonesdebugs) {
            //Iterate over each stonesdebug
            foreach ($stonesdebugs as $stonesdebug) {
                $status = fwrite ($bf,start_tag("stonesdebug",$level,true));
                //Print stonesdebug contents
                fwrite ($bf,full_tag("ANSWER",$level+1,false,$stonesdebug->answer));
                fwrite ($bf,full_tag("TOLERANCE",$level+1,false,$stonesdebug->tolerance));
                //Now backup stonesdebug_units
                $status = question_backup_stonesdebug_units($bf,$preferences,$question,7);
                $status = fwrite ($bf,end_tag("stonesdebug",$level,true));
            }
    ...
    function restore($old_question_id,$new_question_id,$info,$restore) {
        ...
        //Get the stonesdebug array
        if (isset($info['#']['stonesdebug'])) {
            $stonesdebugs = $info['#']['stonesdebug'];
        } else {
            $stonesdebugs = array();
        }

        //Iterate over stonesdebugs
        for($i = 0; $i < sizeof($stonesdebugs); $i++) {
            $num_info = $stonesdebugs[$i];

            //Now, build the question_stonesdebug record structure
            $stonesdebug = new stdClass;
            $stonesdebug->question = $new_question_id;
            $stonesdebug->answer = backup_todb($num_info['#']['ANSWER']['0']['#']);
            $stonesdebug->tolerance = backup_todb($num_info['#']['TOLERANCE']['0']['#']);

            //We have to recode the answer field
            $answer = backup_getid($restore->backup_unique_code,"question_answers",$stonesdebug->answer);
            if ($answer) {
                $stonesdebug->answer = $answer->new_id;
            }

            //The structure is equal to the db, so insert the question_stonesdebug
            $newid = insert_record ("question_stonesdebug", $stonesdebug);
            ...
            //Now restore stonesdebug_units
            $status = question_restore_stonesdebug_units ($old_question_id,$new_question_id,$num_info,$restore);
...
question_register_questiontype(new question_stonesdebug_qtype());
?>
