<?php  // $Id$
/**
 * Defines the editing form for the stonesdebug question type.
 *
...
/**
 * stonesdebug editing form definition.
 */
class question_edit_stonesdebug_form extends question_edit_form {
...
//------------------------------------------------------------------------------------------
        ...
        $this->add_per_answer_fields($mform, get_string('answerno', 'qtype_stonesdebug', '{no}'),
                $creategrades->gradeoptions);
//------------------------------------------------------------------------------------------
        $repeated = array();
        $repeated[] =& $mform->createElement('header', 'unithdr', get_string('unithdr', 'qtype_stonesdebug', '{no}'));
        ...
        $this->repeat_elements($repeated, $repeatsatstart, array(), 'nounits', 'addunits', 2, get_string('addmoreunitblanks', 'qtype_stonesdebug'));
    ...
    function validation($data, $files) {
        ...
        foreach ($answers as $key => $answer) {
            ...
            if ($trimmedanswer != '') {
                ...
                if (!(is_numeric($trimmedanswer) || $trimmedanswer == '*')) {
                    $errors["answer[$key]"] = get_string('answermustbenumberorstar', 'qtype_stonesdebug');
                }
        ...
        if ($answercount == 0) {
            $errors['answer[0]'] = get_string('notenoughanswers', 'qtype_stonesdebug');
        }
        if ($maxgrade == false) {
        ...
        if (isset($data['unit'])) {
            foreach ($data['unit'] as $key => $unit) {
                ...
                if ($trimmedunit!='' && in_array($trimmedunit, $alreadyseenunits)) {
                    $errors["unit[$key]"] = get_string('errorrepeatedunit', 'qtype_stonesdebug');
                    if (trim($data['multiplier'][$key]) == '') {
                        $errors["multiplier[$key]"] = get_string('errornomultiplier', 'qtype_stonesdebug');
                    }
                } else {
