<?php
/**
 * The question type class for the stonesgame question type.
 *
...
/**
 * The stonesgame question class
 */
class stonesgame_qtype extends default_questiontype {
    ...
    function save_question_options($question) {
        ...
        if(!$answers){
            $result->notice = get_string("failedloadinganswers", "qtype_stonesgame");
            return $result;
        }
        ...
        $update = true;
        $options = get_record("qtype_stonesgame", "question", $question->id);
        if (!$options) {
            $update = false;
        ...
        if ($update) {
            if (!update_record("qtype_stonesgame", $options)) {
                $result->error = "Could not update stonesgame options! (id=$options->id)";
                return $result;
            }
        } else {
            if (!insert_record("qtype_stonesgame", $options)) {
    ...
    function delete_question($questionid) {
        delete_records("qtype_stonesgame", "question", $questionid);
        return true;
    }
    ...
    function print_question_formulation_and_controls(&$question, &$state, $cmoptions, $options) {
        ...
        include("$CFG->dirroot/question/type/stonesgame/display.html");
    ...
    function restore($old_question_id,$new_question_id,$info,$restore) {
        ...
        //Iterate over stonesgame
        for($i = 0; $i < sizeof($stonesgames); $i++) {
            $mul_info = $stonesgames[$i];

            //Now, build the qtype_ubhotspots record structure
            $stonesgame = new stdClass;
            $stonesgame->question = $new_question_id;
            $stonesgame->hseditordata = backup_todb($mul_info['#']['HSEDITORDATA']['0']['#']);

            //The structure is equal to the db, so insert the question_shortanswer
            $newid = insert_record ("qtype_stonesgame",$stonesgame);
...
question_register_questiontype(new stonesgame_qtype());
?>
